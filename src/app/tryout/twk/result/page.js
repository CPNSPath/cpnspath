...
export default function TWKResult() {
  ...
  const STATS = [
    ...
    { label: "Benar", value: data.correct, cls: "text-[#16A34A]" },
    { label: "Salah", value: data.wrong, cls: "text-[#DC2626]" },
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC]">
      ...
      <section className="bg-[#172554] py-12 sm:py-16">
        <Container>
          <h1 className="heading-1 text-white mb-2">Hasil TWK</h1>
          <p className="body-base text-[#93C5FD]">Tes Wawasan Kebangsaan</p>
        </Container>
      </section>
        ...
        <Container>
          <div className="max-w-3xl mx-auto space-y-8">
            ...
            <Link href="/tryout/twk">
              <Button variant="ghost" size="sm">← Kembali ke Tryout TWK</Button>
            </Link>
                    ...
                    <details key={i} className="group">
                        ...
                        <Card variant="default" className="p-4 hover:shadow-md transition-shadow">
                          <div className="flex items-start gap-3">
                            <span className={`shrink-0 inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold text-white ${
                              isEmpty ? "bg-[#94A3B8]" : isCorrect ? "bg-[#16A34A]" : "bg-[#DC2626]"
                            }`}>
                              {i + 1}
                            </span>
                      ...
                      <div className="mt-1 ml-4 pl-6 space-y-1.5 pb-2">
                        {q.options.map((opt, index) => {
                          let cls = "text-[#64748B]"
                          let suffix = ""
                          if (index === correctAnswer) { cls = "text-[#16A34A] font-semibold"; suffix = " ✔" }
                          if (index === userAnswer && index !== correctAnswer) { cls = "text-[#DC2626]"; suffix = " (Jawaban Anda)" }
