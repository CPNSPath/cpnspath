    ...
    <DashboardLayout>
      ...
      <div style={{ background: "#fff", borderRadius: 12, padding: "16px 20px", marginBottom: 24, border: "1px solid #e2e8f0" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ flex: 1 }}>
            <p style={{ fontSize: "0.85rem", fontWeight: 600, color: "#334155" }}>TKP — Minimal {PASSING_GRADE}</p>
                ...
                <details key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                  <summary style={{ cursor: "pointer", listStyle: "none", padding: "14px 8px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      <span style={{ flexShrink: 0, fontSize: "0.65rem", fontWeight: 600, padding: "2px 8px", borderRadius: 999, background: statusBg, color: statusColor, textTransform: "uppercase" }}>
                        {statusLabel}
                      </span>
                  ...
                  <div style={{ padding: "8px 8px 16px 48px", display: "flex", flexDirection: "column", gap: 6 }}>
                    {q.options.map((opt, idx) => {
                      ...
                      if (isUser && !isIdeal) {
                        bg = optScore >= 4 ? "rgba(22,163,74,0.06)" : optScore >= 3 ? "rgba(234,179,8,0.08)" : "rgba(220,38,38,0.08)"
                        border = `1px solid ${optScore >= 4 ? "rgba(22,163,74,0.3)" : optScore >= 3 ? "rgba(234,179,8,0.3)" : "rgba(220,38,38,0.3)"}`
                        color = optScore >= 4 ? "#16a34a" : optScore >= 3 ? "#ca8a04" : "#dc2626"
                        fw = 600
                      }

                      return (
                        <div key={idx} style={{ padding: "8px 14px", borderRadius: 8, background: bg, border, display: "flex", alignItems: "center", gap: 10 }}>
                          <span style={{ flexShrink: 0, width: 22, height: 22, borderRadius: "50%", background: isIdeal ? "#16a34a" : isUser ? (optScore >= 3 ? "#ca8a04" : "#dc2626") : "#e2e8f0", color: isIdeal || isUser ? "#fff" : "#64748b", fontSize: "0.7rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>
                            {String.fromCharCode(65 + idx)}
                          </span>
                          <span style={{ fontSize: "0.85rem", color, fontWeight: fw, flex: 1 }}>{opt}</span>
                          <span style={{ flexShrink: 0, fontSize: "0.7rem", fontWeight: 700, color: optScore === 5 ? "#16a34a" : optScore >= 3 ? "#ca8a04" : "#94a3b8", background: optScore === 5 ? "rgba(22,163,74,0.1)" : optScore >= 3 ? "rgba(234,179,8,0.1)" : "rgba(148,163,184,0.1)", padding: "2px 8px", borderRadius: 999 }}>
                            {optScore} poin
                          </span>
                          {isIdeal && (
                            <span style={{ flexShrink: 0, fontSize: "0.6rem", color: "#16a34a", fontWeight: 700 }}>★ Ideal</span>
                          )}
                          {isUser && !isIdeal && (
                            <span style={{ flexShrink: 0, fontSize: "0.6rem", color: "#64748b", fontWeight: 600 }}>← Anda</span>
                          )}
                        </div>
                      )
                    })}
