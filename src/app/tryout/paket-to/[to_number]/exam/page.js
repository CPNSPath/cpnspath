...
export default function PaketTOExam() {
  ...
  // Cek akses + status + set in_progress + persistent timer
  useEffect(() => {
    async function checkAccess() {
      ...
      // Persistent timer — simpan start_time di localStorage
      const timerKey = `to_${toNumber}_start_time`
      ...
      if (remaining <= 0) {
        setChecking(false)
        setTime(0)
        // Langsung submit kalau waktu habis
        setTimeout(() => { submittedRef.current = false; submitExam(true) }, 500)
        return
      }
  ...
  // Timer — mulai hanya setelah time di-set
  useEffect(() => {
    if (checking || questionsLoading || allQuestions.length === 0 || time === null) return
  ...
  }, [checking, questionsLoading, allQuestions.length, time !== null])

  // Anti back button — trap dengan history state
  useEffect(() => {
    if (checking) return
    // Push dummy state supaya back button gak langsung keluar
    window.history.pushState({ examLock: true }, "")

    function handlePopState(e) {
      if (submittedRef.current) return
      // User tekan back → tanya konfirmasi
      ...
      if (confirmLeave) {
        ...
        // User batal keluar → push state lagi biar tetap di halaman
        window.history.pushState({ examLock: true }, "")
      }
    }
  ...
  useEffect(() => {
    ...
    function handlePageHide() {
      if (submittedRef.current) return
      // User beneran close/refresh — mark completed via sendBeacon
      try {
  ...
  async function submitExam(force = false) {
    ...
    localStorage.removeItem(`to_${toNumber}_start_time`)
    ...
    <div className="exam-wrap" style={{ minHeight: "100vh", background: "#F8FAFC", display: "flex", flexDirection: "row", height: "100vh", overflow: "hidden" }}>
      ...
      <div className="exam-question" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#F8FAFC", overflowY: "auto", minWidth: 0 }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
          ...
          <div style={{ background: time !== null && time <= 600 ? "rgba(220,38,38,0.1)" : "rgba(23,37,84,0.08)", borderRadius: 8, padding: "6px 14px", display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: "0.75rem", color: time !== null && time <= 600 ? "#dc2626" : "#475569" }}>⏱</span>
            <span style={{ fontSize: "1rem", fontWeight: 700, color: time !== null && time <= 600 ? "#dc2626" : "#172554", fontVariantNumeric: "tabular-nums" }}>{formatTime()}</span>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: "24px 32px" }}>
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "28px", marginBottom: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
            <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 20 }}>
              ...
              <p style={{ fontSize: "1rem", color: "#0f172a", lineHeight: 1.7, fontWeight: 500, flex: 1, paddingTop: 4, whiteSpace: "pre-line" }}>{allQuestions[current].question}</p>
            </div>
      ...
      <div className="exam-sidebar" style={{ width: "280px", flexShrink: 0, background: "#fff", borderLeft: "1px solid #e2e8f0", display: "flex", flexDirection: "column", padding: "20px", overflowY: "auto" }}>
        <div style={{ background: time !== null && time <= 600 ? "rgba(220,38,38,0.06)" : "#f8fafc", border: `1px solid ${time !== null && time <= 600 ? "rgba(220,38,38,0.2)" : "#e2e8f0"}`, borderRadius: 12, padding: "14px", marginBottom: 14, textAlign: "center" }}>
          ...
          <p style={{ fontSize: "1.75rem", fontWeight: 800, color: time !== null && time <= 600 ? "#dc2626" : "#172554", fontVariantNumeric: "tabular-nums" }}>{formatTime()}</p>
        </div>
