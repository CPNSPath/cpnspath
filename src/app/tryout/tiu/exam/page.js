...
export default function TIUExam() {
  ...
  useEffect(() => {
    ...
    function handlePopState(e) {
      ...
      if (confirmLeave) { submitExam(true) }
      else { window.history.pushState({ examLock: true }, "") }
    }
  ...
  async function submitExam(force = false) {
    ...
    shuffledQuestions.forEach((q, i) => {
      if (currentAnswers[i] === undefined) { empty++ }
      else if (currentAnswers[i] === q.answer) { score += 5; correct++ }
      else { wrong++ }
    })
    ...
    const resultData = {
      ...
      answers: currentAnswers, questions: shuffledQuestions
    }
    ...
    localStorage.removeItem("tiu_start_time")
      ...
      <div className="exam-question" style={{ flex: 1, display: "flex", flexDirection: "column", background: "#F8FAFC", overflowY: "auto", minWidth: 0 }}>
        <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "12px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 10 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ background: SUBTEST_COLOR + "20", color: SUBTEST_COLOR, borderRadius: 8, padding: "5px 12px", fontSize: "0.78rem", fontWeight: 700 }}>TIU — Soal {current + 1}</div>
          ...
          <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", padding: "28px", marginBottom: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {shuffledQuestions[current].options.map((opt, i) => (
                ...
                >
                  <span style={{ flexShrink: 0, width: 26, height: 26, borderRadius: "50%", background: answers[current] === i ? "rgba(255,255,255,0.2)" : "#f1f5f9", color: answers[current] === i ? "#fff" : "#64748b", fontSize: "0.75rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center" }}>{String.fromCharCode(65 + i)}</span>
                  {opt}
                </button>
              ))}
            </div>
